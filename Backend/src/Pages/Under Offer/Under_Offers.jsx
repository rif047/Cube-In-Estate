...
const columnPermissions = {
    Admin: [
        "client", "customer", "name", "code", "location",
...
export default function Under_Offers() {
    ...
    const handleTickSubmit = async () => {
        try {
            ...
            await axios.patch(`${import.meta.env.VITE_API_URL}/api/${EndPoint}/status/${selectedRow._id}`, updateData);
    ...
    const handleCancelSubmit = async () => {
        try {
            await axios.patch(`${import.meta.env.VITE_API_URL}/api/${EndPoint}/cancel/${selectedRow._id}`, { status: "Pending" });
        ...
        { accessorFn: row => `${row.sell_price} tk`, key: "sell_price", header: 'Target', size: 80 },
        {
            ...
            Cell: ({ cell }) => {
                ...
                return Array.isArray(images) && images.length > 0 ? (
                    <img src={`${import.meta.env.VITE_API_URL}/api/Images/${EndPoint}/${images[0]}`}
